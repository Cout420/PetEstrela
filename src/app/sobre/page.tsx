...
import { aboutPageContent as initialContent } from '@/lib/about-content';
...
type AboutPageContent = typeof initialContent;
...
export default function AboutPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [aboutData, generalData] = await Promise.all([
      ...
      if (generalData) {
        ...
        setGeneralContent({ whatsappLink: 'https://wa.me/551142405253' });
      }
      setIsLoading(false);
    }
    fetchData();
       ...
       <section className="bg-muted/30 py-16 md:py-24">
        <div className="container mx-auto grid grid-cols-1 items-center gap-8 px-4 md:grid-cols-2">
          <div className="animate-fade-in text-center md:text-left">
            ...
            <p className="mt-4 mx-auto max-w-xl text-lg text-muted-foreground md:mx-0">
              {content.headerDescription}
            </p>
          </div>
           <div className="animate-scale-in relative hidden h-80 w-full overflow-hidden rounded-lg md:block lg:h-96">
              <Image
